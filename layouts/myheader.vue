    ...
    <div class="wrapper">
      ...
      <nuxt-link to="/">
        <div class="left-wrapper v-link selected">
          <img style="width:50px" width="50" height="50" src="~assets/images/logo1.jpg">
          <span class="text">预约挂号平台</span>
        </div>
      </nuxt-link>
      <!--搜索框-->
...
export default {
  ...
  mounted() {
    //注册全局事件
    window.loginEvnet = new Vue();
    //监听登录事件
    loginEvnet.$on('loginDialogEvent', function () {
  ...
  methods: {
    ...
    login() {
      ...
      userInfo.toLogin(this.userInfo).then(response => {
    ...
    getCodeFun() {
      ...
      sms.toSend(this.userInfo.phone).then(response => {
        this.timeDown();
      }).catch(e => {
    ...
    weixinLogin() {
      ...
      //初始化微信相关参数
      weixin.genQrConnect().then(response => {
        var obj = new WxLogin({
          self_redirect: true,
