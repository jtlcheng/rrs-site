      ...
      <div class="order-detail">
        ...
        <div class="rule-wrapper mt40">
          ...
          <div>1、请确认就诊人信息是否准确，若填写错误将无法取号就诊，损失由本人承担；<br>
            ...
            4、预约挂号支持自费患者使用身份证预约，同时支持医保患者使用社保卡在平台预约挂号。请于就诊当日，携带预约挂号所使用的有效身份证件到院取号；<br>
...
export default {
  ...
  methods: {
    init() {
      orderInfo.getOrders(this.orderId).then(response => {
    ...
    pay(){
      ...
      weixin.createNative(this.orderId)
        .then(response=>{
    ...
    queryPayStatus(orderId){
      weixin.queryPayStatus(orderId)
        .then(response=>{

          console.log(response)
          if (response.data ==='支付中'){
            return
          }
          //清楚定时器效果
    ...
    cancelOrder(){
      this.$confirm('确定取消预约吗?','提示',{
        ...
        //点击确定远程调用
        return orderInfo.cancelOrder(this.orderId)
      }).then((response)=>{
