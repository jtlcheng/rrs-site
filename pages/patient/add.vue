          ...
          <div class="content-wrapper">
            <el-form :model="patient" label-width="110px" label-position="left" ref="patient" :rules="validateRules">
              ...
              <el-form-item prop="address" label="详细地址：">
                <el-input v-model="patient.address" placeholder="应公安机关要求，请填写真实住址" class="input v-input"/>
              </el-form-item>
            </el-form>
          </div>
          <div class="sub-title">
...
export default {
    ...
    return {
      ...
      props: {
        ...
        async lazyLoad (node, resolve) {
          const { level } = node
          //异步获取省市区
          dict.findByParentId(level ? node.value : '86').then(response => {
  ...
  methods: {
    ...
    fetchDataById(id) {
      ...
      patient.getPatientId(id).then(response => {
    ...
    getDict() {
      dict.findByDictCode("CertificatesType").then(response => {
    ...
    saveData() {
      ...
      this.submitBnt = '正在提交...'
      patient.savePatient(this.patient).then(response => {
    ...
    updateData() {
      ...
      this.submitBnt = '正在提交...'
      patient.updatePatient(this.patient).then(response => {
        this.$message.success("提交成功")
        window.location.href = '/patient'
      }).catch(e => {
        this.submitBnt = '保存'
