    ...
    <div class="page-container">
      <div class="hospital-source-list">
        <div class="header-wrapper" style="justify-content:normal">
          ...
          <div class="split"></div>
          <div>{{ baseMap.binDepartmentName }}</div>
        </div>
        <div class="title mt20"> {{ baseMap.departmentName }}</div>
        <!-- 号源列表 #start -->
        <div class="mt60">
          ...
          <div class="calendar-list-wrapper">
            ...
            <div :class="'calendar-item '+item.curClass" style="width: 124px;"
                 v-for="(item, index) in bookingScheduleRuleList" :key="item.id"
...
export default {
  ...
  methods: {
    ...
    getBookingScheduleRule() {
      hospital.getBookingSchedule(this.page, this.limit, this.hoscode, this.depcode).then(response => {
        this.bookingScheduleRuleList = response.data.bookingScheduleRuleList
        ...
        if (this.workDate == null) {
          this.workDate = this.bookingScheduleRuleList[0].workDate
        }
        //判断当天是否停止预约 status == -1 停止预约
        if(this.workDate == this.getCurDate()) {
          this.pageFirstStatus = this.bookingScheduleRuleList[0].status
        } else {
    ...
    findScheduleList() {
      hospital.findScheduleList(this.hoscode, this.depcode, this.workDate).then(response => {
    ...
    dealClass() {
      //处理样式
      for (let i = 0; i < this.bookingScheduleRuleList.length; i++) {
        // depNumber -1:无号 0：约满 >0：有号
        let curClass = this.bookingScheduleRuleList[i].availableNumber == -1 ? 'gray space' : this.bookingScheduleRuleList[i].availableNumber == 0 ? 'gray' : 'small small-space'
        curClass += i == this.activeIndex ? ' selected' : ''
        this.bookingScheduleRuleList[i].curClass = curClass
      }
    },
